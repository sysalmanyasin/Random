...
-- ══════════════════════════════════════════════════════════════
-- SHARED INVENTORY (Dropbox → Supabase, pull-only) — see the
-- sync-inventory-from-dropbox Edge Function. Replaces the old
-- model where inventory lived only in each device's local
-- IndexedDB, refreshed only when whoever was logged in on that
-- device happened to pull from Dropbox. Once the Edge Function is
-- deployed and scheduled, every logged-in user — Main or
-- Sub-Auditor, any device — reads the same inventory, refreshed
-- centrally, with a real "last synced" timestamp instead of a
-- per-device guess.
--
-- Nothing here is written by the app's own client code: only the
-- Edge Function writes, using its service-role key (which bypasses
-- RLS entirely) — that's why there's no insert/update/delete policy
-- for 'authenticated' below at all, only a read policy. The
-- Dropbox access token itself never appears in this file or in any
-- client-side code; it lives solely in the Edge Function's secret
-- store (see the deployment notes alongside that function).
-- ══════════════════════════════════════════════════════════════
...
-- One row per completed sync — lets every client show a real "Inventory
-- last refreshed at X" instead of each device guessing from its own
-- last pull.
...
-- Every logged-in user reads the shared inventory — this is
-- deliberately broader than is_main_auditor(): a Sub-Auditor counting
-- an assignment needs the same live product data a Main Auditor sees,
-- which is the whole point of moving this off per-device local storage.
drop policy if exists "inventory read all staff" on inventory_products;
create policy "inventory read all staff" on inventory_products for select
  using (is_access_valid());
drop policy if exists "sync log read all staff" on inventory_sync_log;
create policy "sync log read all staff" on inventory_sync_log for select
  using (is_access_valid());

create index if not exists idx_inventory_products_company on inventory_products(company);
